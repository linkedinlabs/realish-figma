...
/**
 * Generate a formatted number, random, but within a weighted range.
 ...
 * @param {Array} options.breakCeilings A random integer is initially
 * picked within the weight range and then compared against the array of weights to
 * determine the range of numbers to randomly pick within. This makes it more likely that the
 * chosen number falls within a scatter of the break-points, factoring in the weight of each
 * breakpoint (`breakCeiling`).
 * @param {number} options.startingInt An optional starting point integer. Zero will be used
 * if one is not supplied.
 * @param {string} options.textPluralized A string of representative text for 0 and greater than 1
 * (i.e. “10 people”).
 * @param {string} options.textSingular A string of representative text for 1 (i.e. ”1 person”).
 *
...
/**
 * Generates a random number formatted as an alumni string
...
/**
 * Generates a random number formatted as a attendees count
...
/**
 * Generates a random number formatted as a connections string
...
/**
 * Generates a formatted, random date between now and `120` days in the future (set
...
/**
 * Generates a formatted, random domain name using the `companies` dataset.
...
/**
 * Generates a formatted, random email. The domain name is generated by the
...
/**
 * Generates a the filepath to a random image, based on assignment
 * (company or person).
 *
 * @kind function
...
/**
 * Generates a random number formatted as a followers count
...
/**
 * Generates a random hashtag by picking a base phrase and then applying
...
/**
 * Generates a random number formatted as a members count
...
/**
 * Generates a random profile headline from a few different variables.
...
/**
 * Picks a publishing frequency weighted toward daily and monthly.
 *
 * @kind function
...
/**
 * Generate a random timestamp string from minutes to 6 months. The specific time
...
/**
 * Generates a random string based on the supplied `assignment`. In most cases
...
/**
 * Manages the interface between the data sets and text nodes. Randomizes data
...
export default class Data {
  ...
  /**
   * Generates a random string based on an assigned type. The type may be supplied
